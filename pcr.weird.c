...
int main(int argc, char *argv[])
{
	...
	*/
	...
	if(Confidence != 0) {
		ci = CIBeta(Zc,gamma,y,Confidence);
		if(ci == NULL) {
			fprintf(stderr,"couldn't get CI for PC regr\n");
			Confidence = 0;
		}
		FreeArray2D(Zc);
	}
	...
	*/
	...
	/*
	 * do lower and upper separately
	 */
	if(Confidence != 0) {
		/*
		 * make array for untransform with intercept
		 */
		bc = MakeArray2D(b_star->ydim+1,2);
		if(bc == NULL) {
			exit(1);
		}
		/*
		 * 1-D array for untransform of PCR regression
		 */
		bci = MakeArray1D(ci->ydim);
		if(bci == NULL) {
			exit(1);
		}
		/*
		 * lower first
		 */
		for(i=0; i < bci->ydim; i++) {
			bci->data[i*bci->xdim+0] = ci->data[i*ci->xdim+0];
		}
		bv = MultiplyArray2D(u_s,bci);
		if(bv == NULL) {
			fprintf(stderr,"couldn't get lower CI betas\n");
			exit(1);
		}
		b0 = UnscaleB0(y_bar,bv,cs);
		bc->data[0*bc->xdim+0] = b0;
		for(i=1; i < bc->ydim; i++) {
			bc->data[i*bc->xdim+0] = 
			bv->data[(i-1)*bv->xdim+0]
			 / cs->data[1*cs->xdim+(i-1)];
		}
		/*
		 * then upper
		 */
		for(i=0; i < bci->ydim; i++) {
			bci->data[i*bci->xdim+0] = ci->data[i*ci->xdim+1];
		}
		FreeArray2D(bv);
		bv = MultiplyArray2D(u_s,bci);
		if(bv == NULL) {
			fprintf(stderr,"couldn't get upper CI betas\n");
			exit(1);
		}
		b0 = UnscaleB0(y_bar,bv,cs);
		bc->data[0*bc->xdim+1] = b0;
		for(i=1; i < bc->ydim; i++) {
			bc->data[i*bc->xdim+1] = 
			bv->data[(i-1)*bv->xdim+0]
			 / cs->data[1*cs->xdim+(i-1)];
		}
		printf("%0.2f CI on PCR estimate\n",Confidence);
		PrintArray2D(bc);
		FreeArray2D(bc);
		FreeArray2D(bci);
		FreeArray2D(bv);
